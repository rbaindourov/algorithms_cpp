...
function solution(length, width, height) {
    ...
    // Create an array of boxes, each box represented by its sorted dimensions
    for (let i = 0; i < n; i++) {
        boxes.push([length[i], width[i], height[i]].sort((a, b) => a - b));
    }

    // Sort boxes based on their smallest dimension
...
const width = [1, 3, 2];
const height = [1, 3, 2];
console.log(solution(length, width, height)); // true
